  ...
  self.addListener('raw', function (message) {
    ...
    switch (message.command) {
      ...
      case 'NOTICE':
        ...
        break
      case 'MODE':
        if (self.opt.debug) { util.log('MODE: ' + message.args[0] + ' sets mode: ' + message.args[1]) }
        ...
        modeList.forEach(function (mode) {
          ...
          if (mode in self.prefixForMode) {
            ...
            if (channel.users.hasOwnProperty(modeArg)) {
              if (adding) {
                if (channel.users[modeArg].indexOf(self.prefixForMode[mode]) === -1) { channel.users[modeArg] += self.prefixForMode[mode] }
      ...
      case 'NICK':
        ...
        if (self.opt.debug) { util.log('NICK: ' + message.nick + ' changes nick to ' + message.args[0]) }
      ...
      case 'PRIVMSG':
        ...
        if (self.opt.debug && to == self.nick) { util.log('GOT MESSAGE from ' + from + ': ' + text) }
        break
      case 'INVITE':
        from = message.nick
        to = message.args[0]
      ...
      case 'err_umodeunknownflag':
        if (self.opt.showErrors) { util.log('\u001b[01;31mERROR: ' + util.inspect(message) + '\u001b[0m') }
        break
      ...
      default:
        if (message.commandType == 'error') {
          ...
          if (self.opt.debug) { util.log('\u001b[01;31mUnhandled message: ' + util.inspect(message) + '\u001b[0m') }
          break
        }
    }
  })
